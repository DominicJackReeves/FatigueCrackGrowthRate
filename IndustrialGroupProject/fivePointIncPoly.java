public class fivePointIncPoly	{
	private double ar;
	private double dadn;
	private double x;
	private double deltaK;
	public fivePointIncPoly(){}
	public fivePointIncPoly(double[] a, double[] N, double deltaP)	{
		//Declaring variables for terms used further on.
		double c1	 = .5*(N[4]+N[0]);
		double c2	 = .5*(N[4]-N[0]);
		double sx	 = 0;
		double sx2	 = 0;
		...
		//This loop generates the values of the variables using pre-given equations. 
		for(int i=0; i<5; i++){
			double x = (N[i]-c1)/c2;
			double y = a[i];
			sx  = sx + x;
		...
		double Denom = 5*Term1-sx*Term2+sx2*Term3;
		...
		double Numer3= 5*Term4 - sx*Term5 +sx2 * Term6;
		...
		double Numer4= 5*Term7 - sx*Term8 +sx2 * Term9;
		...
		//A loop determing values for rss and tss
		for(int i=0; i<5; i++){
			double x = (N[i]-c1)/c2;
		...
		//calculating final results for central point a[2]
		//ar is fitted crack size
		//dadn is da/dN
		double r2 = 1- rss/tss;
		dadn = bb2/c2 + 2*bb3*(N[4]-c1)/(c2*c2);
		x = (N[2]-c1)/c2;
